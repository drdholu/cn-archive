...
// metadata for data being transferred
struct Header{
...
char NoneUse[20]; // buffer that's not used anywhere
int HEADLEN = sizeof(header);
int oa = 0; //overwrite all existing files during file transfer process.
// oa = 1 -> overwrites all files.

struct Parameters {
    char symlnkmod; // stores parameters related to symbolic handling
};

//* symlink -> symbolic link, pointer to the target file or directory
...
void rdsymlink(char *file, DIR *dir)
{
    ...
    struct stat st; // info of each file
    char tpath[PATH_LEN] = { '\0' }; // buffer stores abs path of each file
...
// status of a file and handles user input to decide whether to overwrite the file, quit or overwrite all files.
...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
                ...
                header.type == *SYMLNK_F)
        {
            ...
            // [TYPE][FLAG][LEN0][LEN1]...[LENn][FILENAME/CONTENT...]
            for(i=0; i<END; i++)
                header.len[i] = buf[i+2];   
